...
const retrieve = async () => {
    ...
    Data.forEach((docs, index) => {
        const { Doctor: { Days, Name, Timings } } = docs
        console.log(`
            Names = ${Name}
            Days = ${Days}
            Timings = ${Timings}
            `)
        doctors.innerHTML += `<option value="name-${index}">${Name}</option>`
        date.innerHTML += `<option value="day-${index}">${Days[index]}</option>`
        timing.innerHTML += `<option value="time-${index}">${Timings}</option>`
